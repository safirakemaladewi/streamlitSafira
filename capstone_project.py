...
Angka konsumsi ikan nasional pada tahun 2020 sebesar 54,56 kg/kapita. Angka ini naik 0,1% dibanding tahun sebelumnya yang 
sebesar 54,5 kg/kapita. Jika dibandingkan pada 2010, maka angka konsumsi ikan nasional telah meningkat hingga 79%, dengan 
...
perusahaan budidaya, dengan Angka Konsumsi Ikan (kg per kapita) dengan koefisien korelsi sebesar 0,78. 
Keduanya menunjukkan korelasi kuat karena nilai koefisien korelasi > 7. Oleh karena itu, kita bisa menyimpulkan bahwa, 
...
"""
Grafik di atas menampilkan distribusi probabilitas kemungkinan masing-masing data Angka Konsumsi Ikan (AKI) dan Pengeluaran per Kapita di Indonesia.
Berdasarkan visualiasi yang ditampilkan serta uji normalitas dengan Shapiro Wilk diperoleh informasi bahwa kedua data tersebut 
tidak terdistribusi normal (p-value < 0,05). Hal ini turut memvalidasi bahwa konsumsi ikan di Indonesia tidak merata antar wilayah.
...
def display_map(df):
    ...
    for feature in choropleth.geojson.data['features']:
        ...
        feature['properties']['Kind'] = 'Expenditure Ratio : ' + str (df.loc[district_name, 'Percentage'] if district_name in list(df.index) else 'N/A')  
...
- Expenditure Ratio: Rasio Pengeluaran Ikan dengan Pengeluaran per Kapita Disesuaikan (Ribu Rupiah/Orang/Tahun)\n
- AKI: Angka Konsumsi Ikan (Kg/Kapita/Tahun) pada setiap Provinsi
"""
st.caption(cap_map)
...
kunci perekonomian nasional saat ini. Rendahnya konsumsi ikan masyarakat Indonesia khususnya di Pulau Jawa dapat disebabkan oleh preferensi 
konsumen yang cenderung untuk memilih konsumsi daging (Tengker, 2017). Dari sektor pemerintah, dapat lebih menggalakkan lagi program Gerakan 
Memasyarakatkan Makan Ikan (Gemarikan) untuk menyadarkan masyarakat tentang pentingnya konsumsi ikan.
